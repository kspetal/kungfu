		...
		<view class="section">
			...
			<view class="section-body">
				...
				<view class="form-item">
					...
					<picker mode="date"
						@change="(e) => { form.start_time = e.detail.value; calculateDays(); }">
						<view class="picker">
				...
				<view class="form-item">
					...
					<picker mode="date"
						@change="(e) => { form.end_time = e.detail.value; calculateDays(); }">
						<view class="picker">
	...
	export default {
		...
		onLoad(options) {
			if (options.id) {
				...
				this.form.start_time = `${year}-${month}-${day}`
				...
				this.form.end_time = `${nextYear}-${nextMonth}-${nextDay}`
						...
						res = await uniCloud.callFunction({
							...
							data: {
								project: {
									_id: this.id,
									...this.form,
									deposit: this.form.deposit ? Number(this.form.deposit) : 0,
									total_fee: this.form.total_fee ? Number(this.form.total_fee) : 0
								}
							}
						})
					} else {
